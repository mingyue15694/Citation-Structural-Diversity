...
def build_temp_graph(G, embedding_files_folder, paper_ids, output_file):
    ...
    with open(output_file, mode, encoding='utf-8') as json_file:
        ...
        for node_id in G:
            ...
            if Similarity2:
                # 计算相似度数据的下四分位数（25%）、中位数（50%）和上四分位数（75%）
                quartiles2 = np.percentile(Similarity2, [25, 50, 75, 87.5, 90, 95, 98])

                # 计算上四分位和下四分位之间所有值的平均值
                avg_between_quartiles2 = np.mean(
                    [sim for sim in Similarity2 if quartiles2[0] < sim < quartiles2[2]])

                # 将这些统计值作为相似度阈值
                similarity_thresholds2 = [avg_between_quartiles2]
...
output_folder = 'year_venue_50_div4'
