...
def build_temp_graph(G, embedding_files_folder, output_file):
    ...
    with open(output_file, mode, encoding='utf-8') as json_file:
        ...
        for node_id in G:
            ...
            # if len(out_neighbors) == 0 or len(out_neighbors) < 5:
            #     continue
            ...
            similarity_threshold_text1 = []
            similarity_threshold_text2 = []
            ...
            if Similarity2:
                ...
                # 计算相似度数据的平均值
                avg_similarity2 = np.mean(Similarity2)

                # 计算上四分位和下四分位之间所有值的平均值
                filtered_values = [sim for sim in Similarity2 if quartiles2[0] < sim < quartiles2[2]]
                if filtered_values:  # 检查列表是否为空
                    avg_between_quartiles2 = np.mean(filtered_values)
                else:
                    avg_between_quartiles2 = avg_similarity2  # 或者其他适当的默认值

                # 使用 extend 方法一次性添加多个值
                similarity_threshold_text = [avg_between_quartiles2]
