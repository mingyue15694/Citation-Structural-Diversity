...
def load_embedding(embedding_files_folder, node_id):
    ...
    embedding_tensor = torch.tensor(embedding, dtype=torch.float32).to('cpu')
    ...
    with open(output_file, mode, encoding='utf-8') as json_file:
        ...
        for node_id in G:
            ...
            if Similarity2:
                ...
                # 将这些统计值作为相似度阈值
                similarity_thresholds2 = [avg_similarity2]
                # similarity_thresholds2 = [quartiles2[1], avg_similarity2, avg_between_quartiles2]
            else:
                # 如果没有相似度数据，设置一个默认的阈值
                similarity_thresholds2 = [0.0]

            all_similarity_thresholds =  similarity_thresholds2
...
output_folder = 'F:/mingyue/specter/pythonProject1/seven_div/year_venue_50_div6'
