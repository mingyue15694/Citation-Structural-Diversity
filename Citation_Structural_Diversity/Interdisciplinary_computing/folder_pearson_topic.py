...
# 结构多样性值的文件
div_folder = 'F:/mingyue/specter/pythonProject1/pubmed_div6'
...
for filename in os.listdir(div_folder):
    if filename.endswith(".json"):
        print(filename)
        # 初始化多样性值和主题数量
        diversity_values = {f"diversity_values_{i}": [] for i in range(1, 11)}
        diversity_number = []
        topic_number = {}
        paper_ids = []
        ...
        for i in range(1, 11):
            ...
            # print(filename_key)
            # 将结果写入 JSON 文件
            with open('DIV_results_topic6.json', 'a', encoding='utf-8') as f:
