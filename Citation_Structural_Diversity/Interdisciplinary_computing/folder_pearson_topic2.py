...
# 提取主题的文件
topic_folder = 'F:/mingyue/specter/pythonProject1/Extract_Output_2'

# 结构多样性值的文件
div_folder = 'F:/mingyue/specter/pythonProject1/pubmed_div'

# 遍历结构多样性值文件夹中的所有文件
for filename in os.listdir(div_folder):
    if filename.endswith(".json"):
        ...
        topic_number = {}
        paper_ids = []
        file_path = os.path.join(div_folder, filename)
        ...
        # 读取主要主题的数量
        for filename1 in os.listdir(topic_folder):
            if filename1.endswith(".json"):
                file_path = os.path.join(topic_folder, filename1)
                with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for i in range(1, 2):
            ...
            # print(filename_key)
            # 将结果写入 JSON 文件
            with open('DIV_results_topic1.json', 'a', encoding='utf-8') as f:
