...
def load_embedding(embedding_file_path, node_id):
    ...
    # embedding_file_path = f'"{embedding_file_path}"'
...
def build_temp_graph(G, embedding_file_path, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as json_file:
        ...
        for node_id in G:
            ...
            print(node_id)
            print(type(node_id))  # 打印 node_id 的类型
            ...
            for node_temp_id in G_temp:
                if node_temp_id not in embeddings:  # 如果节点的嵌入向量尚未计算过
                    # 如果向量没被计算，则不处理
                    if node_temp_id not in extract_ids:
                        continue
            ...
            if Similarity1:
                ...
                # 计算上四分位和下四分位之间所有值的平均值
                filtered_values = [sim for sim in Similarity1 if quartiles1[0] < sim < quartiles1[2]]
                if filtered_values:  # 检查列表是否为空
                    avg_between_quartiles1 = np.mean(filtered_values)
                else:
                    avg_between_quartiles1 = avg_similarity1  # 或者其他适当的默认值

                # 使用 extend 方法一次性添加多个值
                similarity_threshold_text = [avg_between_quartiles1]

                # 将这些统计值作为相似度阈值
                # similarity_thresholds1 = [quartiles1[0], quartiles1[1], quartiles1[2]]
                similarity_threshold = [quartiles1[2]]
            else:
                # 如果没有相似度数据，设置一个默认的阈值
                similarity_threshold= [0.0]
                similarity_threshold_text = [0.0]
                ...
                for out_neighbor2 in out_neighbors:
                    if out_neighbor1 != out_neighbor2:
                        ...
                        if common_successors:
                            if not G_copy.has_edge(out_neighbor1, out_neighbor2):

                                embedding_u = embeddings.get(out_neighbor2)
                                embedding_v = embeddings.get(out_neighbor1)
                                if embedding_u is not None and embedding_v is not None:
                                    similarity = cosine_similarity(embedding_u, embedding_v)
                                    if similarity > similarity_threshold_text:
                                        G_copy.add_edge(out_neighbor1, out_neighbor2, color='green',
                                                        style='dotted')
...
output_folder = 'F:/mingyue/specter/pythonProject1/pubmed_div6'
...
for filename in os.listdir(data_text_folder):
    if filename.endswith(".json"):
        ...
        base_path = 'F:/mingyue/specter/pythonProject1/pubmed_embeddings'  # 正斜杠也可以
        embedding_file_path = os.path.join(base_path, f'embedding_{filename}')
        # 替换反斜杠为正斜杠
        embedding_file_path = embedding_file_path.replace('\\', '/')
