...
data_folder = 'F:/mingyue/specter/pythonProject1/Extract_Output_1'
G = build_citation_network(data_folder)

# 结构多样性值的文件夹
date_file = 'Hum_Brain_Mapp_div_4.json'
...
diversity_values = {f"diversity_values_{i}": [] for i in range(1, 101)}
venue_values = []
paper_ids = []
...
# 初始化
paper_ids = []  # 用于存储所有的 paper_id
diversity_values = {f"diversity_values_{i}": [] for i in range(1, 101)}  # 假设有最多100个diversity值
...
for i in range(1, 101):
    ...
    # # 打印每组的引用量均值（上四分位数和下四分位数的均值）
    # def print_quartile_citations(grouped_citations, context):
    #     for group in grouped_citations:
    #         citations = grouped_citations[group]
    #         if len(citations) < 2:
    #             mean_citation = np.mean(citations)  # 直接使用原始数据的均值
    #         else:
    #             # 计算四分位数
    ...
    #             mean_citation = quartile_mean
    #         print(
    #             f"{context} {group.capitalize()} Diversity Group Quartile Mean Citations: {mean_citation:.3f}")
    ...
    # 将每个结果逐个写入 JSON 文件
    with open('DIV_results_mean_4.json', 'a', encoding='utf-8') as f:
